...
(function(){
  ...
  this.include = function(){
    ...
    // Resolve SocialCalc.js path robustly (supports nested and flattened node_modules)
    var socialcalcPath;
    try {
      socialcalcPath = require.resolve('socialcalc/dist/SocialCalc.js');
    } catch (_) {
      try { socialcalcPath = require.resolve('socialcalc/SocialCalc.js'); } catch (_) {}
    }
    if (!socialcalcPath) {
      var nestedDist = RealBin + "/node_modules/socialcalc/dist/SocialCalc.js";
      var nested = RealBin + "/node_modules/socialcalc/SocialCalc.js";
      if (fs.existsSync(nestedDist)) socialcalcPath = nestedDist;
      else if (fs.existsSync(nested)) socialcalcPath = nested;
    }
    if (!socialcalcPath) {
      var topDist = RealBin + "/../socialcalc/dist/SocialCalc.js";
      var top = RealBin + "/../socialcalc/SocialCalc.js";
      if (fs.existsSync(topDist)) socialcalcPath = topDist;
      else if (fs.existsSync(top)) socialcalcPath = top;
    }
    if (!socialcalcPath) { throw "Cannot find SocialCalc.js"; }
    this.get((ref$ = {}, ref$[BASEPATH + "/static/socialcalc.js"] = function(){
      this.response.type('application/javascript');
      return this.response.sendfile(socialcalcPath);
    }, ref$));
        ...
        if (/^=/.exec(room) && cbMultiple) {
          ...
          return SC._get(room, IO, function(arg$){
            ...
            if (snapshot) {
              ...
              if (type === Csv && !this$.response.headersSent) {
    ...
    // Aliases for multi-sheet export via '=' prefix
    this.get((ref$ = {}, ref$[BASEPATH + "/=:room.xlsx"] = ExportJ('xlsx'), ref$));
